...
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}  Port Forwarding + Local Backend    ${NC}"
echo -e "${BLUE}======================================${NC}\n"
...
echo -e "  • External services can still reach backend via LoadBalancer or ngrok"
echo ""
...
# Start port forwards for services the local backend needs
...
if [ -n "$BACKEND_LB_IP" ]; then
    ...
    echo -e "${YELLOW}For webhooks, you can:${NC}"
    echo -e "  1. Use ngrok for webhook endpoint only"
    echo -e "  2. Or webhooks won't work (same as regular port-forward)"
    BACKEND_PUBLIC_URL=""
fi

echo ""
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}  Next Steps: Start Local Backend    ${NC}"
echo -e "${BLUE}======================================${NC}"
echo ""
...
echo ""
echo -e "${GREEN}✓ Backend will run locally with live code changes!${NC}"
...
# Create stop script
cat > scripts/stop-port-forward-local-backend.sh << 'EOF'
#!/bin/bash
...
chmod +x scripts/stop-port-forward-local-backend.sh

echo -e "${YELLOW}Note: Port forwarding processes are running in the background${NC}"
echo -e "${YELLOW}To stop them, run: ./scripts/stop-port-forward-local-backend.sh${NC}"
echo ""

# Keep script running
echo -e "${BLUE}Press Ctrl+C to stop all port forwards${NC}"
trap "bash scripts/stop-port-forward-local-backend.sh; exit 0" INT
